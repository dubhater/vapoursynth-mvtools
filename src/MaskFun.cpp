...
void MakeVectorOcclusionMaskTime(MVClipBalls *mvClip, int nBlkX, int nBlkY, double dMaskNormFactor, double fGamma, int nPel, uint8_t *occMask, int occMaskPitch, int time256, int blkSizeX, int blkSizeY) { // analyse vectors field to detect occlusion
...
void VectorMasksToOcclusionMaskTime(uint8_t *VXMask, uint8_t *VYMask, int nBlkX, int nBlkY, double dMaskNormFactor, double fGamma, int nPel, uint8_t *occMask, int occMaskPitch, int time256, int blkSizeX, int blkSizeY) { // analyse vectors field to detect occlusion
    memset(occMask, 0, nBlkX * nBlkY);
    int time4096X = time256 * 16 / blkSizeX;
    int time4096Y = time256 * 16 / blkSizeY;
    double occnorm = 10 / dMaskNormFactor / nPel;
...
void MakeVectorOcclusionMask(MVClipBalls *mvClip, int nBlkX, int nBlkY, double dMaskNormFactor, double fGamma, int nPel, uint8_t *occMask, int occMaskPitch) { // analyse vectors field to detect occlusion
    ...
    for (int by = 0; by < nBlkY; by++) {
        for (int bx = 0; bx < nBlkX; bx++) {
            ...
            if (bx > 0) { // left neighbor
                ...
                int vx1 = block1.GetMV().x;
                if (vx1 > vx)
                    occlusion += (vx1 - vx); // only positive (abs)
            }
            if (bx < nBlkX - 1) { // right neighbor
                int i1 = i + 1;
                const FakeBlockData &block1 = mvClip->GetBlock(0, i1);
...
void VectorMasksToOcclusionMask(uint8_t *VXMask, uint8_t *VYMask, int nBlkX, int nBlkY, double dMaskNormFactor, double fGamma, int nPel, uint8_t *occMask) { // analyse vectors field to detect occlusion

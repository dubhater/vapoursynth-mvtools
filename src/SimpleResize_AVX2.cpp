...
static FORCE_INLINE void simpleResize_int16_t_vertical_4px_avx2(int16_t *workp, const int16_t *srcp1, const int16_t *srcp2, int x, const __m128i &dwords_weights) {
    __m128i top = _mm_loadl_epi64((const __m128i *)&srcp1[x]);
    __m128i bottom = _mm_loadl_epi64((const __m128i *)&srcp2[x]);
    __m128i pixels = _mm_unpacklo_epi16(bottom, top);

    __m128i dst = _mm_madd_epi16(pixels, dwords_weights);
    dst = _mm_add_epi32(dst, _mm_set1_epi32(simple_resize_weight_half));
    dst = _mm_srai_epi32(dst, simple_resize_weight_shift);
    dst = _mm_packs_epi32(dst, dst);
    _mm_storel_epi64((__m128i *)&workp[x], dst);
}
...
void simpleResize_int16_t_avx2(const SimpleResize *simple, int16_t *dstp, int dst_stride, const int16_t *srcp, int src_stride) {
    ...
    for (int y = 0; y < simple->dst_height; y++) {
        ...
        for (int x = 0; x < src_width_sse2; x += pixels_per_iteration)
            simpleResize_int16_t_vertical_4px_avx2(workp, srcp1, srcp2, x, dwords_weights_v);

        if (src_width_sse2 < simple->src_width)
            simpleResize_int16_t_vertical_4px_avx2(workp, srcp1, srcp2, simple->src_width - pixels_per_iteration, dwords_weights_v);

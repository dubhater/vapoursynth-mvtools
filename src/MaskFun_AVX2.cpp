...
static FORCE_INLINE __m256i lookup_AVX2(const int16_t *VXFull, const int16_t *VYFull, const PixelType *pref, int w, const __m256i &dwords_time256, const __m256i &dwords_ref_pitch, const __m256i &dwords_hoffsets) {
    ...
    if (sizeof(PixelType) == 1)
        gathered = _mm256_and_si256(gathered, _mm256_set1_epi32(255));
//    else
        // The one extra 16 bit pixel doesn't matter. It will be multiplied by 0.
...
static FORCE_INLINE void FlowInter_8px_AVX2(
    ...
    if (sizeof(PixelType) == 1) {
        dstF0 = _mm256_and_si256(dstF0, dwords_255);
        dstB0 = _mm256_and_si256(dstB0, dwords_255);
    }
    ...
    __m256i dstF_maskf_inv = _mm256_mullo_epi16(dstF, maskf_inv);
    __m256i dstB_maskb_inv = _mm256_mullo_epi16(dstB, maskb_inv);
    if (sizeof(PixelType) == 2) {
        __m256i tempf = _mm256_mulhi_epu16(dstF, maskf_inv);
        __m256i tempb = _mm256_mulhi_epu16(dstB, maskb_inv);
        tempf = _mm256_slli_epi32(tempf, 16); /// pmulld is 10 cycles, but it doesn't require shift and or
        tempb = _mm256_slli_epi32(tempb, 16);
        dstF_maskf_inv = _mm256_or_si256(dstF_maskf_inv, tempf);
        dstB_maskb_inv = _mm256_or_si256(dstB_maskb_inv, tempb);
    }

    __m256i dstF0_maskb = _mm256_mullo_epi16(dstF0, maskb);
    __m256i dstB0_maskf = _mm256_mullo_epi16(dstB0, maskf);
    if (sizeof(PixelType) == 2) {
        __m256i tempf = _mm256_mulhi_epu16(dstF0, maskb);
        __m256i tempb = _mm256_mulhi_epu16(dstB0, maskf);
        tempf = _mm256_slli_epi32(tempf, 16);
        tempb = _mm256_slli_epi32(tempb, 16);
        dstF0_maskb = _mm256_or_si256(dstF0_maskb, tempf);
        dstB0_maskf = _mm256_or_si256(dstB0_maskf, tempb);
    }
...
static FORCE_INLINE void FlowInterExtra_8px_AVX2(
    ...
    if (sizeof(PixelType) == 2) {
        __m256i dstF_hi = _mm256_mulhi_epu16(dstF, maskf_inv);
        __m256i dstB_hi = _mm256_mulhi_epu16(dstB, maskb_inv);
        dstF_hi = _mm256_slli_epi32(dstF_hi, 16);
        dstB_hi = _mm256_slli_epi32(dstB_hi, 16);
        dstF = _mm256_or_si256(dstF, dstF_hi);
        dstB = _mm256_or_si256(dstB, dstB_hi);
    }

    dstF = _mm256_add_epi32(dstF, dwords_255);
    dstB = _mm256_add_epi32(dstB, dwords_255);

    dstF = _mm256_add_epi32(dstF, _mm256_mullo_epi16(medianBB, maskf));
    dstB = _mm256_add_epi32(dstB, _mm256_mullo_epi16(medianFF, maskb));
    if (sizeof(PixelType) == 2) {
        dstF = _mm256_add_epi32(dstF, _mm256_slli_epi32(_mm256_mulhi_epu16(medianBB, maskf), 16));
        dstB = _mm256_add_epi32(dstB, _mm256_slli_epi32(_mm256_mulhi_epu16(medianFF, maskb), 16));
    }

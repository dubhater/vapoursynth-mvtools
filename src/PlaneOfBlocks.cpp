...
void PlaneOfBlocks::RecalculateMVs(MVClipBalls &mvClip, MVFrame *_pSrcFrame, MVFrame *_pRefFrame,
    ...
    for (blky = 0; blky < nBlkY; blky++) {
        ...
        for (int iblkx = 0; iblkx < nBlkX; iblkx++) {
            ...
            int saduv = (chroma) ? SADCHROMA(pSrc[1], nSrcPitch[1], GetRefBlockU(predictor.x, predictor.y), nRefPitch[1]) + SADCHROMA(pSrc[2], nSrcPitch[2], GetRefBlockV(predictor.x, predictor.y), nRefPitch[2]) : 0;
...
void PlaneOfBlocks::PseudoEPZSearch() {
    ...
    saduv = (chroma) ? SADCHROMA(pSrc[1], nSrcPitch[1], GetRefBlockU(0, 0), nRefPitch[1]) + SADCHROMA(pSrc[2], nSrcPitch[2], GetRefBlockV(0, 0), nRefPitch[2]) : 0;
    ...
    saduv = (chroma) ? SADCHROMA(pSrc[1], nSrcPitch[1], GetRefBlockU(globalMVPredictor.x, globalMVPredictor.y), nRefPitch[1]) + SADCHROMA(pSrc[2], nSrcPitch[2], GetRefBlockV(globalMVPredictor.x, globalMVPredictor.y), nRefPitch[2]) : 0;
    ...
    saduv = (chroma) ? SADCHROMA(pSrc[1], nSrcPitch[1], GetRefBlockU(predictor.x, predictor.y), nRefPitch[1]) + SADCHROMA(pSrc[2], nSrcPitch[2], GetRefBlockV(predictor.x, predictor.y), nRefPitch[2]) : 0;
...
void PlaneOfBlocks::ExpandingSearch(int r, int s, int mvx, int mvy) {
...
void PlaneOfBlocks::Hex2Search(int i_me_range) { //adopted from x264
    int dir = -2;
    int bmx = bestMV.x;
    int bmy = bestMV.y;
...
void PlaneOfBlocks::CrossSearch(int start, int x_max, int y_max, int mvx, int mvy) { // part of umh  search

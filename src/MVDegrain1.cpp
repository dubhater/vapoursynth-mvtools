...
static void selectFunctions(MVDegrain1Data *d) {
    ...
    if (d->isse)
    {
        ...
        {
            case 32:
                if (nBlkSizeY==16) {          d->OVERSLUMA = Overlaps32x16_mmx;  d->DEGRAINLUMA = Degrain1_sse2<32,16>;
                    if (yRatioUV==2) {         d->OVERSCHROMA = Overlaps16x8_mmx; d->DEGRAINCHROMA = Degrain1_sse2<16,8>;         }
                    else {                     d->OVERSCHROMA = Overlaps16x16_mmx;d->DEGRAINCHROMA = Degrain1_sse2<16,16>;         }
                } else if (nBlkSizeY==32) {    d->OVERSLUMA = Overlaps32x32_mmx;  d->DEGRAINLUMA = Degrain1_sse2<32,32>;
                    if (yRatioUV==2) {            d->OVERSCHROMA = Overlaps16x16_mmx; d->DEGRAINCHROMA = Degrain1_sse2<16,16>;         }
                    else {                        d->OVERSCHROMA = Overlaps16x32_mmx; d->DEGRAINCHROMA = Degrain1_sse2<16,32>;         }
                } break;
            case 16:
                if (nBlkSizeY==16) {          d->OVERSLUMA = Overlaps16x16_mmx; d->DEGRAINLUMA = Degrain1_sse2<16,16>;
                    if (yRatioUV==2) {            d->OVERSCHROMA = Overlaps8x8_mmx; d->DEGRAINCHROMA = Degrain1_sse2<8,8>;         }
                    else {                        d->OVERSCHROMA = Overlaps8x16_mmx;d->DEGRAINCHROMA = Degrain1_sse2<8,16>;         }
                } else if (nBlkSizeY==8) {    d->OVERSLUMA = Overlaps16x8_mmx;  d->DEGRAINLUMA = Degrain1_sse2<16,8>;
                    if (yRatioUV==2) {            d->OVERSCHROMA = Overlaps8x4_mmx; d->DEGRAINCHROMA = Degrain1_sse2<8,4>;         }
                    else {                        d->OVERSCHROMA = Overlaps8x8_mmx; d->DEGRAINCHROMA = Degrain1_sse2<8,8>;         }
                } else if (nBlkSizeY==2) {    d->OVERSLUMA = Overlaps16x2_mmx;  d->DEGRAINLUMA = Degrain1_sse2<16,2>;
                    if (yRatioUV==2) {         d->OVERSCHROMA = Overlaps8x1_mmx; d->DEGRAINCHROMA = Degrain1_sse2<8,1>;         }
                    else {                        d->OVERSCHROMA = Overlaps8x2_mmx; d->DEGRAINCHROMA = Degrain1_sse2<8,2>;         }
                }
                break;
            case 4:
                d->OVERSLUMA = Overlaps4x4_mmx;    d->DEGRAINLUMA = Degrain1_mmx<4,4>;
            ...
            case 8:
            default:
                if (nBlkSizeY==8) {           d->OVERSLUMA = Overlaps8x8_mmx;    d->DEGRAINLUMA = Degrain1_sse2<8,8>;
                    if (yRatioUV==2) {            d->OVERSCHROMA = Overlaps4x4_mmx;  d->DEGRAINCHROMA = Degrain1_mmx<4,4>;         }
                    else {                        d->OVERSCHROMA = Overlaps4x8_mmx;  d->DEGRAINCHROMA = Degrain1_mmx<4,8>;         }
                }else if (nBlkSizeY==4) {     d->OVERSLUMA = Overlaps8x4_mmx;    d->DEGRAINLUMA = Degrain1_sse2<8,4>;
                    if (yRatioUV==2) {            d->OVERSCHROMA = Overlaps4x2_mmx;    d->DEGRAINCHROMA = Degrain1_mmx<4,2>;         }
                    else {                        d->OVERSCHROMA = Overlaps4x4_mmx;  d->DEGRAINCHROMA = Degrain1_mmx<4,4>;         }
                }
        }
    }
    else

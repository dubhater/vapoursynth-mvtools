...
static void VS_CC mvdegrain2Init(VSMap *in, VSMap *out, void **instanceData, VSNode *node, VSCore *core, const VSAPI *vsapi) {
    MVDegrain2Data *d = (MVDegrain2Data *) * instanceData;
    vsapi->setVideoInfo(d->vi, 1, node);
}


static const VSFrameRef *VS_CC mvdegrain2GetFrame(int n, int activationReason, void **instanceData, void **frameData, VSFrameContext *frameCtx, VSCore *core, const VSAPI *vsapi) {
    MVDegrain2Data *d = (MVDegrain2Data *) * instanceData;
    ...
    if (activationReason == arInitial) {
                ...
                for (int by=0; by<nBlkY; by++)
                {
                    ...
                    for (int bx=0; bx<nBlkX; bx++)
                    {
                        ...
                        d->DEGRAINLUMA(pDstCur[0] + xx, nDstPitches[0], pSrcCur[0]+ xx, nSrcPitches[0],
                                pB, npB, pF, npF, pB2, npB2, pF2, npF2, WSrc, WRefB, WRefF, WRefB2, WRefF2);
        ...
        // -----------------------------------------------------------------------------
        // CHROMA plane U
        if (!(YUVplanes & UPLANE & nSuperModeYUV))
        ...
        else
        {
            if (nOverlapX==0 && nOverlapY==0)
            {
                for (int by=0; by<nBlkY; by++)
                {
                    ...
                    {
                        int i = by*nBlkX + bx;
                        const uint8_t * pBU, *pFU, *pB2U, *pF2U;
                        int npBU, npFU, npB2U, npF2U;
                        ...
                        d->DEGRAINCHROMA(pDstCur[1] + (xx>>1), nDstPitches[1], pSrcCur[1]+ (xx>>1), nSrcPitches[1],
                                pBU, npBU, pFU, npFU, pB2U, npB2U, pF2U, npF2U, WSrc, WRefB, WRefF, WRefB2, WRefF2);
        ...
        else
        {
            if (nOverlapX==0 && nOverlapY==0)
            {
                for (int by=0; by<nBlkY; by++)
                {
                    ...
                    {
                        int i = by*nBlkX + bx;
                        const uint8_t * pBV, *pFV, *pB2V, *pF2V;
                        ...
                        d->DEGRAINCHROMA(pDstCur[2] + (xx>>1), nDstPitches[2], pSrcCur[2]+ (xx>>1), nSrcPitches[2],
                                pBV, npBV, pFV, npFV, pB2V, npB2V, pF2V, npF2V, WSrc, WRefB, WRefF, WRefB2, WRefF2);
...
static void VS_CC mvdegrain2Free(void *instanceData, VSCore *core, const VSAPI *vsapi) {
    MVDegrain2Data *d = (MVDegrain2Data *)instanceData;
...
static void selectFunctions(MVDegrain2Data *d) {
    const int yRatioUV = d->bleh->yRatioUV;
    const int nBlkSizeX = d->bleh->nBlkSizeX;
    const int nBlkSizeY = d->bleh->nBlkSizeY;
    ...
    if (d->isse)
    {
        ...
        {
            case 32:
                if (nBlkSizeY==16) {          d->OVERSLUMA = mvtools_Overlaps32x16_sse2;  d->DEGRAINLUMA = Degrain2_sse2<32,16>;
                    if (yRatioUV==2) {         d->OVERSCHROMA = mvtools_Overlaps16x8_sse2; d->DEGRAINCHROMA = Degrain2_sse2<16,8>;         }
                    else {                     d->OVERSCHROMA = mvtools_Overlaps16x16_sse2;d->DEGRAINCHROMA = Degrain2_sse2<16,16>;         }
                } else if (nBlkSizeY==32) {    d->OVERSLUMA = mvtools_Overlaps32x32_sse2;  d->DEGRAINLUMA = Degrain2_sse2<32,32>;
                    if (yRatioUV==2) {            d->OVERSCHROMA = mvtools_Overlaps16x16_sse2; d->DEGRAINCHROMA = Degrain2_sse2<16,16>;         }
                    else {                        d->OVERSCHROMA = mvtools_Overlaps16x32_sse2; d->DEGRAINCHROMA = Degrain2_sse2<16,32>;         }
                } break;
            case 16:
                if (nBlkSizeY==16) {          d->OVERSLUMA = mvtools_Overlaps16x16_sse2; d->DEGRAINLUMA = Degrain2_sse2<16,16>;
                    if (yRatioUV==2) {            d->OVERSCHROMA = mvtools_Overlaps8x8_sse2; d->DEGRAINCHROMA = Degrain2_sse2<8,8>;         }
                    else {                        d->OVERSCHROMA = mvtools_Overlaps8x16_sse2;d->DEGRAINCHROMA = Degrain2_sse2<8,16>;         }
                } else if (nBlkSizeY==8) {    d->OVERSLUMA = mvtools_Overlaps16x8_sse2;  d->DEGRAINLUMA = Degrain2_sse2<16,8>;
                    if (yRatioUV==2) {            d->OVERSCHROMA = mvtools_Overlaps8x4_sse2; d->DEGRAINCHROMA = Degrain2_sse2<8,4>;         }
                    else {                        d->OVERSCHROMA = mvtools_Overlaps8x8_sse2; d->DEGRAINCHROMA = Degrain2_sse2<8,8>;         }
                } else if (nBlkSizeY==2) {    d->OVERSLUMA = mvtools_Overlaps16x2_sse2;  d->DEGRAINLUMA = Degrain2_sse2<16,2>;
                    if (yRatioUV==2) {         d->OVERSCHROMA = mvtools_Overlaps8x1_sse2; d->DEGRAINCHROMA = Degrain2_sse2<8,1>;         }
                    else {                        d->OVERSCHROMA = mvtools_Overlaps8x2_sse2; d->DEGRAINCHROMA = Degrain2_sse2<8,2>;         }
                }
                break;
            case 4:
                d->OVERSLUMA = mvtools_Overlaps4x4_sse2;    d->DEGRAINLUMA = Degrain2_mmx<4,4>;
                if (yRatioUV==2) {            d->OVERSCHROMA = Overlaps_C<2,2>;    d->DEGRAINCHROMA = Degrain2_C<2,2>;         }
                else {                        d->OVERSCHROMA = Overlaps_C<2,4>;    d->DEGRAINCHROMA = Degrain2_C<2,4>;         }
                break;
            case 8:
            default:
                if (nBlkSizeY==8) {           d->OVERSLUMA = mvtools_Overlaps8x8_sse2;    d->DEGRAINLUMA = Degrain2_sse2<8,8>;
                    if (yRatioUV==2) {            d->OVERSCHROMA = mvtools_Overlaps4x4_sse2;  d->DEGRAINCHROMA = Degrain2_mmx<4,4>;         }
                    else {                        d->OVERSCHROMA = mvtools_Overlaps4x8_sse2;  d->DEGRAINCHROMA = Degrain2_mmx<4,8>;         }
                }else if (nBlkSizeY==4) {     d->OVERSLUMA = mvtools_Overlaps8x4_sse2;    d->DEGRAINLUMA = Degrain2_sse2<8,4>;
                    if (yRatioUV==2) {            d->OVERSCHROMA = mvtools_Overlaps4x2_sse2;    d->DEGRAINCHROMA = Degrain2_mmx<4,2>;         }
                    else {                        d->OVERSCHROMA = mvtools_Overlaps4x4_sse2;  d->DEGRAINCHROMA = Degrain2_mmx<4,4>;         }
                }
        }
    }
    else
    {
        ...
        {
            case 32:
                if (nBlkSizeY==16) {          d->OVERSLUMA = Overlaps_C<32,16>;  d->DEGRAINLUMA = Degrain2_C<32,16>;
                    if (yRatioUV==2) {            d->OVERSCHROMA = Overlaps_C<16,8>; d->DEGRAINCHROMA = Degrain2_C<16,8>;         }
                    else {                        d->OVERSCHROMA = Overlaps_C<16,16>;d->DEGRAINCHROMA = Degrain2_C<16,16>;         }
                } else if (nBlkSizeY==32) {    d->OVERSLUMA = Overlaps_C<32,32>;   d->DEGRAINLUMA = Degrain2_C<32,32>;
                    if (yRatioUV==2) {            d->OVERSCHROMA = Overlaps_C<16,16>;  d->DEGRAINCHROMA = Degrain2_C<16,16>;         }
                    else {                        d->OVERSCHROMA = Overlaps_C<16,32>;  d->DEGRAINCHROMA = Degrain2_C<16,32>;         }
                } break;
            case 16:
                if (nBlkSizeY==16) {          d->OVERSLUMA = Overlaps_C<16,16>;  d->DEGRAINLUMA = Degrain2_C<16,16>;
                    if (yRatioUV==2) {            d->OVERSCHROMA = Overlaps_C<8,8>;  d->DEGRAINCHROMA = Degrain2_C<8,8>;         }
                    else {                        d->OVERSCHROMA = Overlaps_C<8,16>; d->DEGRAINCHROMA = Degrain2_C<8,16>;         }
                } else if (nBlkSizeY==8) {    d->OVERSLUMA = Overlaps_C<16,8>;   d->DEGRAINLUMA = Degrain2_C<16,8>;
                    if (yRatioUV==2) {            d->OVERSCHROMA = Overlaps_C<8,4>;  d->DEGRAINCHROMA = Degrain2_C<8,4>;         }
                    else {                        d->OVERSCHROMA = Overlaps_C<8,8>;  d->DEGRAINCHROMA = Degrain2_C<8,8>;         }
                } else if (nBlkSizeY==2) {    d->OVERSLUMA = Overlaps_C<16,2>;   d->DEGRAINLUMA = Degrain2_C<16,2>;
                    if (yRatioUV==2) {            d->OVERSCHROMA = Overlaps_C<8,1>;  d->DEGRAINCHROMA = Degrain2_C<8,1>;         }
                    else {                        d->OVERSCHROMA = Overlaps_C<8,2>;  d->DEGRAINCHROMA = Degrain2_C<8,2>;         }
                }
                break;
            case 4:
                d->OVERSLUMA = Overlaps_C<4,4>;    d->DEGRAINLUMA = Degrain2_C<4,4>;
                if (yRatioUV==2) {            d->OVERSCHROMA = Overlaps_C<2,2>;  d->DEGRAINCHROMA = Degrain2_C<2,2>;         }
                else {                        d->OVERSCHROMA = Overlaps_C<2,4>;  d->DEGRAINCHROMA = Degrain2_C<2,4>;         }
                break;
            case 8:
            default:
                if (nBlkSizeY==8) {           d->OVERSLUMA = Overlaps_C<8,8>;    d->DEGRAINLUMA = Degrain2_C<8,8>;
                    if (yRatioUV==2) {            d->OVERSCHROMA = Overlaps_C<4,4>;  d->DEGRAINCHROMA = Degrain2_C<4,4>;         }
                    else {                        d->OVERSCHROMA = Overlaps_C<4,8>;  d->DEGRAINCHROMA = Degrain2_C<4,8>;         }
                }else if (nBlkSizeY==4) {     d->OVERSLUMA = Overlaps_C<8,4>;    d->DEGRAINLUMA = Degrain2_C<8,4>;
                    if (yRatioUV==2) {            d->OVERSCHROMA = Overlaps_C<4,2>;  d->DEGRAINCHROMA = Degrain2_C<4,2>;         }
                    else {                        d->OVERSCHROMA = Overlaps_C<4,4>;  d->DEGRAINCHROMA = Degrain2_C<4,4>;         }
                }
        }
    }
}
...
static void VS_CC mvdegrain2Create(const VSMap *in, VSMap *out, void *userData, VSCore *core, const VSAPI *vsapi) {
    MVDegrain2Data d;
    MVDegrain2Data *data;
    ...
    if (!evil) {
        vsapi->setError(out, std::string("Degrain2: failed to retrieve first frame from super clip. Error message: ").append(errorMsg).c_str());
        vsapi->freeNode(d.super);
        return;
    }
    ...
    for (int i = 0; i < 6; i++)
        if (evil_err[i]) {
            vsapi->setError(out, "Degrain2: required properties not found in first frame of super clip. Maybe clip didn't come from mv.Super? Was the first frame trimmed away?");
            vsapi->freeNode(d.super);
            return;
        }
    ...
    // XXX See if MVClipDicks can avoid throwing.
    try {
    ...
    try {
        d.bleh = new MVFilter(d.mvfw2, "Degrain2", vsapi);
    } catch (MVException &e) {
        vsapi->setError(out, e.what());
        vsapi->freeNode(d.super);
        vsapi->freeNode(d.mvfw);
    ...
    if (!d.mvClipB2->IsBackward() || !d.mvClipB->IsBackward() ||
        ...
        if (!d.mvClipB2->IsBackward())
            ...
            vsapi->setError(out, "Degrain2: mvbw must be generated with isb=True.");
        else if (d.mvClipF->IsBackward())
            vsapi->setError(out, "Degrain2: mvfw must be generated with isb=False.");
        else 
            vsapi->setError(out, "Degrain2: mvfw2 must be generated with isb=False.");
        vsapi->freeNode(d.super);
        vsapi->freeNode(d.mvfw);
        vsapi->freeNode(d.mvbw);
        vsapi->freeNode(d.mvfw2);
    ...
    if (d.mvClipB2->GetDeltaFrame() <= d.mvClipB->GetDeltaFrame() ||
        ...
        if (d.mvClipB2->GetDeltaFrame() <= d.mvClipB->GetDeltaFrame())
            ...
            vsapi->setError(out, "Degrain2: mvfw2 must have greater delta than mvfw.");
        vsapi->freeNode(d.super);
        vsapi->freeNode(d.mvfw);
        vsapi->freeNode(d.mvbw);
        vsapi->freeNode(d.mvfw2);
    ...
    if (d.bleh->nHeight != nHeightS || d.bleh->nHeight != d.vi->height || d.bleh->nWidth != nSuperWidth - d.nSuperHPad * 2 || d.bleh->nWidth != d.vi->width) {
        vsapi->setError(out, "Degrain2: wrong source or super clip frame size.");
        vsapi->freeNode(d.super);
        vsapi->freeNode(d.mvfw);
        vsapi->freeNode(d.mvbw);
        vsapi->freeNode(d.mvfw2);
    ...
    data = (MVDegrain2Data *)malloc(sizeof(d));
    *data = d;

    vsapi->createFilter(in, out, "Degrain2", mvdegrain2Init, mvdegrain2GetFrame, mvdegrain2Free, fmParallel, 0, data, core);
}

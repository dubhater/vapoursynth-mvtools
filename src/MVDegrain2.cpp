...
static inline void normaliseWeights(int &WSrc, int &WRefB, int &WRefF, int &WRefB2, int &WRefF2) {
    WSrc = 256;
            ...
            if (nOverlapX[0] == 0 && nOverlapY[0] == 0) {
                for (int by = 0; by < nBlkY; by++) {
                    ...
                    for (int bx = 0; bx < nBlkX; bx++) {
                        ...
                        normaliseWeights(WSrc, WRefB, WRefF, WRefB2, WRefF2);
...
static void selectFunctions(MVDegrain2Data *d) {
    ...
    if (d->isse)
    {
        ...
        {
            case 32:
                if (nBlkSizeY==16) {          d->OVERS[0] = mvtools_Overlaps32x16_sse2;  d->DEGRAIN[0] = Degrain2_sse2<32,16>;
                    if (yRatioUV==2) {         d->OVERS[1] = mvtools_Overlaps16x8_sse2; d->DEGRAIN[1] = Degrain2_sse2<16,8>;         }
                    else {                     d->OVERS[1] = mvtools_Overlaps16x16_sse2;d->DEGRAIN[1] = Degrain2_sse2<16,16>;         }
                } else if (nBlkSizeY==32) {    d->OVERS[0] = mvtools_Overlaps32x32_sse2;  d->DEGRAIN[0] = Degrain2_sse2<32,32>;
                    if (yRatioUV==2) {            d->OVERS[1] = mvtools_Overlaps16x16_sse2; d->DEGRAIN[1] = Degrain2_sse2<16,16>;         }
                    else {                        d->OVERS[1] = mvtools_Overlaps16x32_sse2; d->DEGRAIN[1] = Degrain2_sse2<16,32>;         }
                } break;
            case 16:
                if (nBlkSizeY==16) {          d->OVERS[0] = mvtools_Overlaps16x16_sse2; d->DEGRAIN[0] = Degrain2_sse2<16,16>;
                    if (yRatioUV==2) {            d->OVERS[1] = mvtools_Overlaps8x8_sse2; d->DEGRAIN[1] = Degrain2_sse2<8,8>;         }
                    else {                        d->OVERS[1] = mvtools_Overlaps8x16_sse2;d->DEGRAIN[1] = Degrain2_sse2<8,16>;         }
                } else if (nBlkSizeY==8) {    d->OVERS[0] = mvtools_Overlaps16x8_sse2;  d->DEGRAIN[0] = Degrain2_sse2<16,8>;
                    if (yRatioUV==2) {            d->OVERS[1] = mvtools_Overlaps8x4_sse2; d->DEGRAIN[1] = Degrain2_sse2<8,4>;         }
                    else {                        d->OVERS[1] = mvtools_Overlaps8x8_sse2; d->DEGRAIN[1] = Degrain2_sse2<8,8>;         }
                } else if (nBlkSizeY==2) {    d->OVERS[0] = mvtools_Overlaps16x2_sse2;  d->DEGRAIN[0] = Degrain2_sse2<16,2>;
                    if (yRatioUV==2) {         d->OVERS[1] = mvtools_Overlaps8x1_sse2; d->DEGRAIN[1] = Degrain2_sse2<8,1>;         }
                    else {                        d->OVERS[1] = mvtools_Overlaps8x2_sse2; d->DEGRAIN[1] = Degrain2_sse2<8,2>;         }
                }
                break;
            case 4:
                d->OVERS[0] = mvtools_Overlaps4x4_sse2;    d->DEGRAIN[0] = Degrain2_sse2<4,4>;
                if (yRatioUV==2) {            d->OVERS[1] = Overlaps_C<2,2>;    d->DEGRAIN[1] = Degrain2_C<2,2>;         }
                else {                        d->OVERS[1] = Overlaps_C<2,4>;    d->DEGRAIN[1] = Degrain2_C<2,4>;         }
                break;
            case 8:
            default:
                if (nBlkSizeY==8) {           d->OVERS[0] = mvtools_Overlaps8x8_sse2;    d->DEGRAIN[0] = Degrain2_sse2<8,8>;
                    if (yRatioUV==2) {            d->OVERS[1] = mvtools_Overlaps4x4_sse2;  d->DEGRAIN[1] = Degrain2_sse2<4,4>;         }
                    else {                        d->OVERS[1] = mvtools_Overlaps4x8_sse2;  d->DEGRAIN[1] = Degrain2_sse2<4,8>;         }
                }else if (nBlkSizeY==4) {     d->OVERS[0] = mvtools_Overlaps8x4_sse2;    d->DEGRAIN[0] = Degrain2_sse2<8,4>;
                    if (yRatioUV==2) {            d->OVERS[1] = mvtools_Overlaps4x2_sse2;    d->DEGRAIN[1] = Degrain2_sse2<4,2>;         }
                    else {                        d->OVERS[1] = mvtools_Overlaps4x4_sse2;  d->DEGRAIN[1] = Degrain2_sse2<4,4>;         }
                }
        }
    }
    else
    {
        ...
        {
            case 32:
                if (nBlkSizeY==16) {          d->OVERS[0] = Overlaps_C<32,16>;  d->DEGRAIN[0] = Degrain2_C<32,16>;
                    if (yRatioUV==2) {            d->OVERS[1] = Overlaps_C<16,8>; d->DEGRAIN[1] = Degrain2_C<16,8>;         }
                    else {                        d->OVERS[1] = Overlaps_C<16,16>;d->DEGRAIN[1] = Degrain2_C<16,16>;         }
                } else if (nBlkSizeY==32) {    d->OVERS[0] = Overlaps_C<32,32>;   d->DEGRAIN[0] = Degrain2_C<32,32>;
                    if (yRatioUV==2) {            d->OVERS[1] = Overlaps_C<16,16>;  d->DEGRAIN[1] = Degrain2_C<16,16>;         }
                    else {                        d->OVERS[1] = Overlaps_C<16,32>;  d->DEGRAIN[1] = Degrain2_C<16,32>;         }
                } break;
            case 16:
                if (nBlkSizeY==16) {          d->OVERS[0] = Overlaps_C<16,16>;  d->DEGRAIN[0] = Degrain2_C<16,16>;
                    if (yRatioUV==2) {            d->OVERS[1] = Overlaps_C<8,8>;  d->DEGRAIN[1] = Degrain2_C<8,8>;         }
                    else {                        d->OVERS[1] = Overlaps_C<8,16>; d->DEGRAIN[1] = Degrain2_C<8,16>;         }
                } else if (nBlkSizeY==8) {    d->OVERS[0] = Overlaps_C<16,8>;   d->DEGRAIN[0] = Degrain2_C<16,8>;
                    if (yRatioUV==2) {            d->OVERS[1] = Overlaps_C<8,4>;  d->DEGRAIN[1] = Degrain2_C<8,4>;         }
                    else {                        d->OVERS[1] = Overlaps_C<8,8>;  d->DEGRAIN[1] = Degrain2_C<8,8>;         }
                } else if (nBlkSizeY==2) {    d->OVERS[0] = Overlaps_C<16,2>;   d->DEGRAIN[0] = Degrain2_C<16,2>;
                    if (yRatioUV==2) {            d->OVERS[1] = Overlaps_C<8,1>;  d->DEGRAIN[1] = Degrain2_C<8,1>;         }
                    else {                        d->OVERS[1] = Overlaps_C<8,2>;  d->DEGRAIN[1] = Degrain2_C<8,2>;         }
                }
                break;
            case 4:
                d->OVERS[0] = Overlaps_C<4,4>;    d->DEGRAIN[0] = Degrain2_C<4,4>;
                if (yRatioUV==2) {            d->OVERS[1] = Overlaps_C<2,2>;  d->DEGRAIN[1] = Degrain2_C<2,2>;         }
                else {                        d->OVERS[1] = Overlaps_C<2,4>;  d->DEGRAIN[1] = Degrain2_C<2,4>;         }
                break;
            case 8:
            default:
                if (nBlkSizeY==8) {           d->OVERS[0] = Overlaps_C<8,8>;    d->DEGRAIN[0] = Degrain2_C<8,8>;
                    if (yRatioUV==2) {            d->OVERS[1] = Overlaps_C<4,4>;  d->DEGRAIN[1] = Degrain2_C<4,4>;         }
                    else {                        d->OVERS[1] = Overlaps_C<4,8>;  d->DEGRAIN[1] = Degrain2_C<4,8>;         }
                }else if (nBlkSizeY==4) {     d->OVERS[0] = Overlaps_C<8,4>;    d->DEGRAIN[0] = Degrain2_C<8,4>;
                    if (yRatioUV==2) {            d->OVERS[1] = Overlaps_C<4,2>;  d->DEGRAIN[1] = Degrain2_C<4,2>;         }
                    else {                        d->OVERS[1] = Overlaps_C<4,4>;  d->DEGRAIN[1] = Degrain2_C<4,4>;         }
                }
        }
    }
